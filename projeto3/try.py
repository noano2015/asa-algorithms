...
for i in range(n_factories):
    ...
    criancas_recebem_i = tuple()
    for k in range(t_kids):
        criancas_recebem_i += (x_k_i_vars[f"x_{k+1}_{i+1}"],)

    problem += (pulp.lpSum(criancas_recebem_i) <= factories_maximum[i], f"restricao1_{i+1}")
...
for k in range(t_kids):
    ...
    crianca_recebe = tuple()
    for i in range(n_factories):
        crianca_recebe += (x_k_i_vars[f"x_{k+1}_{i+1}"],)
    
    problem += (pulp.lpSum(crianca_recebe) <= 1, f"restricao2_{k+1}")
    del crianca_recebe
...
for j in range(m_countries):
    ...
    criancas_de_j = tuple()
    for k in range(t_kids):
        if kids_countries[k] == j+1:
            criancas_de_j += (k+1,)

    crianca_de_j_recebe = tuple()
    for crianca in criancas_de_j:
        for i in range(n_factories):
            crianca_de_j_recebe += (x_k_i_vars[f"x_{crianca}_{i+1}"],)
            
    del criancas_de_j
    problem += ((pulp.lpSum(crianca_de_j_recebe)) >= countries_minimum[j], f"restricao3_{j+1}")
...
for p in range(m_countries):
    ...
    brinquedos_de_p = tuple()
    for i in range(n_factories):
        if factory_country[i] == p+1:
            brinquedos_de_p += (i+1,)

    criancas_estrangeiras = tuple()
    for k in range(t_kids):
        if kids_countries[k] != p+1:
            criancas_estrangeiras += (k+1,)
    
    exportacoes_de_p = tuple()
    for crianca in criancas_estrangeiras:
        for brinquedo in brinquedos_de_p:
            exportacoes_de_p += (x_k_i_vars[f"x_{crianca}_{brinquedo}"],)
    
    del brinquedos_de_p
    del criancas_estrangeiras
    problem += ((pulp.lpSum(exportacoes_de_p)) <= countries_maximum[p], f"restricao4_{p+1}")
...
for k in range(t_kids):
    ...
    not_wished = tuple()
    for i in range(n_factories):
        if (i+1 not in kids_wishes[k]):
            not_wished += (i+1,)
    
    not_wished_but_received = []

    for b in not_wished:
        not_wished_but_received += (x_k_i_vars[f"x_{k+1}_{b}"],)

    del not_wished
    problem += ((pulp.lpSum(not_wished_but_received)) <= 0, f"restricao5_{k+1}")
    del not_wished_but_received


del input_list 

del factories_maximum 
del factory_country 
del countries_maximum
del countries_minimum
del kids_countries
del kids_wishes
